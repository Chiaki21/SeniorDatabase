...
$selectedAge = isset($_GET['age']) ? $_GET['age'] : '';
$birthYear = date('Y') - $selectedAge;
$startDate = $birthYear . '-01-01';
$endDate = $birthYear . '-12-31';
$sqlCount = "SELECT COUNT(*) as total FROM people WHERE (STR_TO_DATE(birthDate, '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate')
             OR (STR_TO_DATE(birthDate, '%m/%d/%Y') BETWEEN '$startDate' AND '$endDate')
             OR (STR_TO_DATE(birthDate, '%m-%d-%Y') BETWEEN '$startDate' AND '$endDate')";
$sqlSelect = "SELECT * FROM people WHERE (STR_TO_DATE(birthDate, '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate')
              OR (STR_TO_DATE(birthDate, '%m/%d/%Y') BETWEEN '$startDate' AND '$endDate')
              OR (STR_TO_DATE(birthDate, '%m-%d-%Y') BETWEEN '$startDate' AND '$endDate') ORDER BY updated_date DESC LIMIT $offset, $recordsPerPage";
...
<head>
    <meta charset="UTF-8" />
    <title>Senior Solutions | Age</title>
    ...
    <section class="home-section">
        <nav>
            <div class="sidebar-button">
                ...
                <span class="dashboard">Age: <?php echo $selectedAge; ?></span>
        ...
        <div class="home-content">
            ...
            <div class="pagination">
                ...
                <a
                    href="?page=1<?php echo $searchQuery ? '&searchQuery=' . urlencode($searchQuery) : ''; ?>&age=<?php echo $selectedAge; ?>">First</a>
                <a
                    href="?page=<?php echo ($currentpage - 1); ?><?php echo $searchQuery ? '&searchQuery=' . urlencode($searchQuery) : ''; ?>&age=<?php echo $selectedAge; ?>">Previous</a>
                <?php endif; ?>
                ...
                <?php else : ?>
                <a
                    href="?page=<?php echo $i; ?><?php echo $searchQuery ? '&searchQuery=' . urlencode($searchQuery) : ''; ?>&age=<?php echo $selectedAge; ?>"><?php echo $i; ?></a>
                <?php endif; ?>
                <?php endfor; ?>

                <?php if ($currentpage < $totalPages) : ?>
                <a
                    href="?page=<?php echo ($currentpage + 1); ?><?php echo $searchQuery ? '&searchQuery=' . urlencode($searchQuery) : ''; ?>&age=<?php echo $selectedAge; ?>">Next</a>
                <a
                    href="?page=<?php echo $totalPages; ?><?php echo $searchQuery ? '&searchQuery=' . urlencode($searchQuery) : ''; ?>&age=<?php echo $selectedAge; ?>">Last</a>
                <?php endif; ?>
            </div>
